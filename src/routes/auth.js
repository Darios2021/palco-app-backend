...
function signAccess(user){
  return jwt.sign({ sub: user.id, email: user.email, name: user.name }, process.env.JWT_ACCESS_SECRET, { expiresIn: ACCESS_TOKEN_TTL })
}
...
r.post('/login', async (req, res, next) => {
  try {
    const { User, RefreshToken } = getDB().sequelize.models
    ...
    return res.json({ accessToken: signAccess(user), user: { id: user.id, name: user.name, email: user.email } })
  } catch (e) { next(e) }
})

r.post('/refresh', async (req, res, next) => {
  try {
    const { RefreshToken, User } = getDB().sequelize.models
    ...
    setRefreshCookie(res, signRefresh({ sub: user.id, jti: newJti }))
    res.json({ accessToken: signAccess(user), user: { id: user.id, name: user.name, email: user.email } })
  } catch (e) { next(e) }
})

r.post('/logout', async (req, res, next) => {
  try {
    const { RefreshToken } = getDB().sequelize.models
    const rt = req.cookies?.rt
    if (rt) {
      try {
        const payload = jwt.verify(rt, process.env.JWT_REFRESH_SECRET)
...
r.get('/me', async (req, res, next) => {
  try {
    ...
    const user = await getDB().sequelize.models.User.findByPk(payload.sub)
